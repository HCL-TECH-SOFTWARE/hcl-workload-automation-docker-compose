...
The HCL Workload Automation container has the following prerequisites:
 ...
 - Create a DB instance and schema
   ...
   For example, use the following command to create a DB2 instance and schema:   
       ...
       docker run --rm hcl-workload-automation-console:9.5.0.03.20210218 cat /opt/dwc/tools/create_database.sql > create_database.sql
...
To create new LTPA token, issue the following command:
     ...
     docker run -i --rm -v <host_dir>:/output hcl-workload-automation-console:9.5.0.03.20210218 /opt/wautils/wa_create_ltpa_keys.sh -p <keys_password>
